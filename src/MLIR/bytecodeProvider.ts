...
class BytecodeFS implements vscode.FileSystemProvider {
  ...
  constructor(mlirContext: MLIRContext) {
    this.mlirContext = mlirContext;
  }
  ...
  rename(
    ...
    vscode.workspace.fs.rename(
      oldUri.with({ scheme: "file" }),
      newUri.with({ scheme: "file" }),
      options,
    );
  }
  ...
  async readFile(uri: vscode.Uri): Promise<Uint8Array> {
    ...
    // it.
    const client = await this.mlirContext.getOrActivateLanguageClient(
      uri,
      "mlir",
    );
    if (!client) {
    ...
    if (resultBuffer.length > 50 * 1024 * 1024) {
      ...
      const message: string = `Failed to open bytecode file "${uri.toString()}". Cannot edit converted bytecode files larger than 50MB.`;
  ...
  async writeFile(
    ...
    const result: ConvertBytecodeResult = await client.sendRequest(
      "mlir/convertToBytecode",
      convertParams,
    );
 ...
 * interpretation.
 */
class BytecodeEditorProvider
  implements vscode.CustomReadonlyEditorProvider<BytecodeDocument>
{
 ...
 */
export function registerMLIRBytecodeExtensions(
