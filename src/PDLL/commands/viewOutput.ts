...
export class ViewPDLLCommand extends Command {
  constructor(context: MLIRContext) {
    super("mlir.viewPDLLOutput", context);
  }

  async execute() {
    const editor = vscode.window.activeTextEditor;
    if (editor.document.languageId != "pdll") return;

    // Check to see if a language client is active for this document.
    const pdllClient = this.context.getLanguageClient(
      editor.document.uri,
      "pdll",
    );
    if (!pdllClient) {
    ...
    const result: ViewOutputResult | undefined = await pdllClient.sendRequest(
      "pdll/viewOutput",
      outputParams,
    );
